...
export default function ModalDetailDeclaration({ declaration, onClose, onModifier, onSupprimer }) {
  ...
  const d = declaration;
  const modifiable = d.statut === "BROUILLON" || d.statut === "REJETEE_DG" || d.statut === "REJETEE_CIL" || d.statut === "EN_ATTENTE";
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[92vh] flex flex-col">
        <div className="bg-green-800 text-white px-6 py-4 rounded-t-2xl flex justify-between items-center flex-shrink-0 sticky top-0 z-10">
          <div>
        ...
        <div className="px-5 py-4 border-t bg-gray-50 flex flex-wrap gap-2 flex-shrink-0 rounded-b-2xl">
          <button onClick={() => telechargerDeclarationPdf(d)} className="px-4 py-2 rounded-lg bg-green-700 text-white text-sm font-medium hover:bg-green-800">
            Télécharger PDF
          </button>
          {modifiable && onModifier && (
            <button onClick={() => { onModifier(d); onClose(); }} className="px-4 py-2 rounded-lg border border-green-600 text-green-700 text-sm font-semibold hover:bg-green-50">
              Modifier
            </button>
          )}
          {onSupprimer && (
            <button onClick={() => { if (window.confirm(`Supprimer la déclaration #${d.idDeclaration} ?`)) { onSupprimer(d.idDeclaration); onClose(); } }} className="px-4 py-2 rounded-lg border border-red-400 text-red-600 text-sm font-semibold hover:bg-red-50">
              Supprimer
            </button>
          )}
          <button onClick={onClose} className="px-4 py-2 rounded-lg border border-gray-300 text-gray-600 text-sm hover:bg-gray-50 ml-auto">
            Fermer
          </button>
        </div>
      </div>
