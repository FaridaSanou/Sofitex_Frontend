        ...
        <div className="px-6 space-y-4 text-sm overflow-y-auto flex-1">
          {etape === 1 && (
            <div className="space-y-3">
              <h4 className="font-bold text-gray-800 text-base">Informations du Traitement</h4>
          ...
          {etape === 2 && (
            <div className="space-y-3">
              <h4 className="font-bold text-gray-800 text-base">Détails & Conformité</h4>
              <div className="bg-green-50 rounded-xl p-4 border border-green-100">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-white rounded-lg p-3"><p className="text-xs text-green-600 font-semibold">Durée conservation</p><p>{traitement.dureeConservation ? `${traitement.dureeConservation} mois` : "—"}</p></div>
                  ...
                  <div className="bg-white rounded-lg p-3 col-span-2"><p className="text-xs text-green-600 font-semibold">Lieu stockage</p><p>{traitement.lieuStockage || "—"}</p></div>
                  <div className="bg-white rounded-lg p-3 col-span-2"><p className="text-xs text-green-600 font-semibold">Session</p><p>{traitement.sessionCollecteId ? `#${traitement.sessionCollecteId}` : "Aucune session"}</p></div>
                </div>
              </div>
            </div>
          )}
          ...
          {etape === 3 && (
            <div className="space-y-3">
              <h4 className="font-bold text-gray-800 text-base">Responsable du Traitement</h4>
              <div className="bg-green-50 rounded-xl p-4 border border-green-100">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-white rounded-lg p-3 col-span-2"><p className="text-xs text-green-600 font-semibold">Nom complet</p><p>{traitement.nomPrenomResponsable || "—"}</p></div>
                  ...
                  <div className="bg-white rounded-lg p-3 col-span-2"><p className="text-xs text-green-600 font-semibold">Email</p><p>{traitement.contactConfidentialite || "—"}</p></div>
                </div>
              </div>
          ...
          {etape < steps.length ? (
            ...
            <div className="flex flex-col gap-2">
              {!traitement.envoyeAuDpo && (
                <button onClick={() => { const dpoId = dpoRequis ? Number(dpoSelection) : undefined; if (dpoRequis && !dpoSelection) return; onEnvoyer(traitement.idTraitement, dpoId).then(() => onClose()); }}
                  disabled={dpoRequis && !dpoSelection}
                  className="w-full px-4 py-2.5 rounded-lg bg-green-700 text-white text-sm font-semibold hover:bg-green-800 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                  <Icon name="send" className="w-4 h-4 rotate-45" /> Envoyer au DPO
                </button>
              )}
              <div className="grid grid-cols-3 gap-2">
                <button onClick={() => telechargerTraitementPdf(traitement)} className="px-4 py-2.5 rounded-lg bg-white border border-gray-300 text-gray-800 text-sm font-medium hover:bg-gray-50">PDF</button>
                {!traitement.envoyeAuDpo && (
                  <button onClick={() => { onModifier?.(traitement); onClose(); }}
                    className="px-4 py-2.5 rounded-lg border border-green-600 text-green-700 text-sm font-semibold hover:bg-green-50">
                    Modifier
                  </button>
                )}
                {!traitement.envoyeAuDpo && (
                  <button onClick={() => { if (window.confirm(`Supprimer le traitement #${traitement.idTraitement} "${traitement.description}" ?`)) { onSupprimer?.(traitement.idTraitement); onClose(); } }}
                    className="px-4 py-2.5 rounded-lg border border-red-400 text-red-600 text-sm font-semibold hover:bg-red-50">
                    Supprimer
                  </button>
                )}
              </div>
