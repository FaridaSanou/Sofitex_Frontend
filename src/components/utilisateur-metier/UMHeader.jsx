...
function NotificationDropdown({ notifications, unreadCount, onMarkRead, onMarkAllRead, onDemandesClick }) {
        ...
        <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-2xl shadow-xl border border-gray-200 z-50 max-h-[70vh] flex flex-col">
          ...
          {unreadCount > 0 && (
            <div className="px-5 py-3 border-t border-gray-100 space-y-2">
              <button onClick={() => { onMarkAllRead(); }} className="w-full py-2 text-xs font-medium text-green-700 bg-green-50 border border-green-200 rounded-xl hover:bg-green-100 transition">
                Tout marquer lu
              </button>
...
export default function UMHeader({ activeSection, sidebarOpen, onToggleSidebar, newSessionCount, onNewSessionClick, onDemandesClick, notifications, unreadCount, onMarkRead, onMarkAllRead }) {
  return (
