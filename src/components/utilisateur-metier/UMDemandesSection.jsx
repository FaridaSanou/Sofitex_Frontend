      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {demandes.map(d => {
              const statut = d.statut || d.statutDemande;
              const enAttente = statut === "EN_COURS";
              return (
                <tr key={d.id} className="hover:bg-green-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    {enAttente
                      ...
                      : statut === "ACCEPTEE"
                        ? <span className="bg-green-100 text-green-700 text-xs font-semibold px-2 py-0.5 rounded-full flex items-center gap-1 w-fit"><Icon name="check" className="w-3 h-3" />Acceptée</span>
                        : <span className="bg-red-100 text-red-700 text-xs font-semibold px-2 py-0.5 rounded-full flex items-center gap-1 w-fit"><Icon name="close" className="w-3 h-3" />Rejetée</span>
                    }
                  </td>
                  <td className="px-4 py-3">
                    {enAttente ? (
                      ...
                      <button onClick={() => onTraiter(d)} className="text-xs bg-gray-100 text-gray-600 px-3 py-1 rounded-lg hover:bg-gray-200 flex items-center gap-1">
                        <Icon name="eye" className="w-3 h-3" /> Voir
                      </button>
                    )}
                  </td>
                </tr>
