...
function DpoDashboard() {
  ...
  const fetchTraitements = useCallback(async (sessionId) => {
    if (!sessionId) { setTraitements([]); return; }
    try {
  ...
  useEffect(() => { fetchTraitements(selectedSessionId); }, [selectedSessionId, fetchTraitements]);
  ...
  const handleChangeStatut = async (id, valeur) => {
    try {
      const res = await api.patch(`/sessions/${id}/statut`, null, { params: { valeur } });
      setSessions((prev) => prev.map((s) => (s.idSession === id ? res.data : s)));
      ...
      {activeTab === "dashboard" && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <StatCard label="Traitements" value={stats.traitements} color="bg-purple-500" />
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="overflow-x-auto">
                ...
                <tbody className="divide-y divide-gray-50">
                  {traitements.map((t) => (
                    <tr key={t.idTraitement} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-5 py-4 text-gray-600 text-sm">#{t.sessionCollecteId || "—"}</td>
                      <td className="px-5 py-4">
              ...
              {traitements.length === 0 && <div className="py-12 text-center text-gray-400 text-sm">Aucun traitement</div>}
            </div>
          </div>
        </div>
      )}
