...
const TYPE_FIELDS = [
  "adresse", "matricule",
  "service", "niveauResponsabilite", "fonction",
  "organisme", "adresseProfessionnelle",
  "idDg",
];
...
const inputCls = (err) =>
  `h-10 px-3 rounded-lg border text-sm outline-none bg-white/70 w-full ${
    err ? "border-red-300 bg-red-50" : "border-gray-300 focus:border-green-500"
  }`;

// ─── Construction du payload selon le type ───────────────────────────────────
const buildPayload = (data, typeChoisi) => {
  const base = {
    nom:             data.nom,
    prenom:          data.prenom,
    email:           data.email,
  ...
  switch (typeChoisi) {
    case "USAGER":
      return { ...base, adresse: data.adresse, matricule: data.matricule };
    case "CIL":
      return { ...base, service: data.service, fonction: data.fonction, niveauResponsabilite: data.niveauResponsabilite };
    case "DPO":
      return { ...base, organisme: data.organisme, adresseProfessionnelle: data.adresseProfessionnelle };
    case "DG":
      return { ...base, idDg: data.idDg };
    case "UTILISATEUR_METIER":
      return { ...base, fonction: data.fonction };
    default:
      return base;
  }
};
...
export default function CreateAccount() {
  ...
  const { register, handleSubmit, watch, trigger, reset, formState: { errors } } = useForm({ mode: "onTouched" });
  ...
  const handleChangeType = (newType) => {
    reset((formValues) => ({
      ...formValues,
      ...Object.fromEntries(TYPE_FIELDS.map((f) => [f, ""])),
    }));
    setTypeChoisi(newType);
  };
  ...
  const handleNextMain = async () => {
    const valid = await trigger(["nom", "prenom", "email", "telephone", "motdepasse", "confirmPwd"]);
  ...
  const onSubmit = async (data) => {
    ...
    setErrGlobal("");
    try {
      const payload = buildPayload(data, typeChoisi);
      ...
      if (!response.ok) {
        const message = result?.message || result?.error || `Erreur ${response.status}`;
        setErrGlobal(message);
        console.error("Erreur serveur :", response.status, result);
        return;
      }
      ...
      setErrGlobal("Impossible de contacter le serveur. Vérifiez que le backend tourne sur le port 8080.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="min-h-screen flex items-center justify-center p-4"
        ...
        <div className="rounded-2xl shadow-2xl p-10 w-full max-w-lg text-center"
          ...
          <div className="flex gap-3">
            <button onClick={() => { setDone(false); setMainStep(0); setSubStep(0); handleChangeType(""); }}
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4"
      ...
      <div className="rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden"
        ...
        {/* ── En-tête ── */}
        <div className="px-6 py-4 text-center" style={{ background: "linear-gradient(135deg, #15803d, #166534)" }}>
              ...
              <>
                ...
                {!typeChoisi && (
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-6">
                      {TYPES.map((t) => (
                        <button key={t.value} type="button" onClick={() => handleChangeType(t.value)}
                ...
                {typeChoisi && (
                  <>
                      ...
                      <>
                        <div className="grid grid-cols-2 gap-4 mb-4">
                          ...
                          <Field label="Fonction" error={errors.fonction}>
                            <input placeholder="Ex : Responsable CIL" className={inputCls(errors.fonction)}
                    ...
                    {typeChoisi === "DG" && (
                      <div className="mb-6">
                        <Field label="Numero profesionnel" error={errors.idDg}>
                          <input placeholder="Ex : 20952610" className={inputCls(errors.idDg)}
                            {...register("idDg", { required: "Champ requis" })} />
                        </Field>
                      </div>
                    )}
                    ...
                    <div style={{ display: "flex", gap: "12px", marginTop: "8px" }}>
                      <button type="button"
                        onClick={() => { handleChangeType(""); setMainStep(0); setSubStep(1); setErrGlobal(""); }}
                        style={{
                          flex: 1, height: "40px",
                          border: "2px solid #15803d", borderRadius: "10px",
